<?php
    class View {
        ...
        * @var string
        */
        static $tpl_dir = TEMPLATE;
        static $tpl_default_dir = "views/static/";
        ...
        * @var unknown_type
        */
        static $black_list = array( '\$this', 'raintpl::', 'self::', '_SESSION', '_SERVER', '_ENV',  'eval', 'exec', 'unlink', 'rmdir','phpinfo' );
        ...
        */
        ...
        function draw( $tpl_name, $return_string = false ){
            ...
            // rain get the output buffer to save the output in the cache or to return it as string
                ...
                if( $this->cache )
                    file_put_contents( $this->tpl['cache_filename'], "<?php if(!class_exists('view')){exit;}?>" . $raintpl_contents );
        ...
        */
        ...
        protected function check_template( $tpl_name ){
            ...
            if( !isset($this->tpl['checked']) ){
                ...
                if(!file_exists($this->tpl['tpl_filename'])) $this->tpl['tpl_filename']          = self::$tpl_default_dir . $tpl_basedir . $tpl_basename . '.' . self::$tpl_ext;    // template filename
                $temp_compiled_filename             = self::$cache_dir . $tpl_basename . "." . md5( $this->tpl['template_directory'] . serialize(self::$config_name_sum));
                $this->tpl['compiled_filename']     = $temp_compiled_filename . '.cache.php';    // cache filename
                $this->tpl['cache_filename']        = $temp_compiled_filename . '.s_' . $this->cache_id . '.cache.php';    // static cache filename
                ...
                if( self::$check_template_update && !file_exists( $this->tpl['tpl_filename'] ) && !preg_match('/http/', $tpl_name) ){
                    $e = die( 'HTML Dosyası bulunamadı ! Dosya Adı : <b>'. $tpl_basename .'.html</b>' );
        ...
        */
        protected function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
            ...
            //compile template
            $template_compiled = "<?php if(!class_exists('view')){exit;}?>" . $this->compileTemplate( $template_code, $tpl_basedir );
            ...
            if( !is_writable( $cache_dir ) )
                throw new View_Exception ('Cache directory ' . $cache_dir . 'doesn\'t have write permission. Set write permission or set RAINTPL_CHECK_TEMPLATE_UPDATE to false. More details on http://www.raintpl.com/Documentation/Documentation-for-PHP-developers/Configuration/');
        ...
        */
        protected function compileCode( $parsed_code ){
            ...
            if( $open_if > 0 ) {
                $e = new ViewTpl_SyntaxException('Error! You need to close an {if} tag in ' . $this->tpl['tpl_filename'] . ' template');
                throw $e->setTemplateFile($this->tpl['tpl_filename']);
            }
            return $compiled_code;
        }
        ...
        */
        public static function single_path_replace ( $matches ){
            $tag  = $matches[1];
            $_    = $matches[2];
            $attr = $matches[3];
            $url  = $matches[4];
        ...
        */
        private function path_replace( $html, $tpl_basedir ){
            ...
            if( self::$path_replace ){
                ...
                return preg_replace_callback( $exp, array(__CLASS__,'single_path_replace'), $html );
        ...
        */
        protected function function_check( $code ){
            ...
            if( count(self::$black_list) && preg_match( $preg, $code, $match ) ){
                ...
                $e = new ViewTpl_SyntaxException('Unallowed syntax in ' . $this->tpl['tpl_filename'] . ' template');
                throw $e->setTemplateFile($this->tpl['tpl_filename'])
                ->setTag($code)
        ...
        *
        * @param View_Exception $e
        * @return string
        */
        protected function printDebug(View_Exception $e){
            if (!self::$debug) {
                throw $e;
            }
            ...
            if ($e instanceof ViewTpl_SyntaxException) {
    ...
    */
    class NotFoundException extends View_Exception{
    }

    /**
    * Exception thrown when syntax error occurs.
    */
    class ViewTpl_SyntaxException extends View_Exception{
        /**
        ...
        * @return ViewTpl_SyntaxException
        */
