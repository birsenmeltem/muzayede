...
jQuery(document).ready(function() {
    ...
    $('[data-ck]').on('change',function() {
        var f = $(this).data('ck');
        var pid = $(this).data('id');
        var val = $(this).prop('checked');
        $.post(baseURL + 'products/changefield', {'f':f,'pid':pid,'val':val});
    })
    $('.secAll').on('change',function() {
        $('.sec').prop('checked',$(this).prop('checked'));
    })
    ...
    $('.remove').click(function (e) {
        ...
        swal.fire({
            title: "Emin misiniz?",
            text: "Kayıt tamamen silinecektir. İşlem geri alınamaz",
            type: "warning",
    ...
    $('.btn-cargoiptal').click(function (e) {
        e.preventDefault();

        var link = $(this).attr('href');
        swal.fire({
            title: "Emin misiniz?",
            text: "Sipariş entegrasyon listesinden çıkartılacaktır ve sipariş durumu değişecektir !",
            type: "info",
            showCancelButton: true,
            confirmButtonColor: '#32b1d6',
    ...
    $('.btn-sendcargo').click(function (e) {
        e.preventDefault();

        var link = $(this).attr('href');
        swal.fire({
            title: "Onaylıyor musunuz?",
            text: "Sipariş entegrasyona aktarılacak ve barkod yazdırılacaktır.",
            type: "info",
            showCancelButton: true,
            confirmButtonColor: '#32b1d6',
            confirmButtonClass: 'btn-success',
    ...
    $('.btn-anadil').click(function (e) {
        e.preventDefault();

        var link = $(this).attr('href');
        swal.fire({
            title: "Emin misiniz?",
            text: "Veri girişi yapıldıktan sonra sistem ana dili değiştirilirse bozulmalar olabilir ve sistem hatalı çalışabilir. Onaylıyor musunuz?",
            type: "info",
            showCancelButton: true,
            confirmButtonColor: '#32b1d6',
            confirmButtonClass: 'btn-success',
    ...
    $('.btn-customer-export').on('click', function() {
        var form = $('#customerexport').serialize();
        $.post(baseURL + 'customers/download',form,function(json) {
            if(json.status != 'success') {
                swal.fire('HATA !',json.message, 'error');
            } else top.location.href = baseURL + 'customers/download/true';
        });
    });

    $('[data-invoice]').on('click', function() {
        var auction = $(this).data('a');
        var user_id = $(this).data('u');
        var tip = $(this).data('t');
        $.post(baseURL + 'orders/parasut/'+auction+'/'+user_id+'/'+tip,function(data) {
            var json = $.parseJSON(data);
            if(json.redirect) {
                window.open(json.redirect,'asd');
                //top.location.href = json.redirect;
            } else {
                swal.fire(json.head,json.message, json.cls);
            }
        });
    });
...
function get_citys(id, target, all) {
    $.post(baseURL + 'index/get_citys', {'country_id':id , 'all':all}, function(data) {
        $(target).html(data);
        $(target).selectpicker('refresh');
    })
}

function rebuildvariants(product_id) {
    $.post(baseURL + 'products/productsvariants',{'id' : product_id}, function(data) {
        if(data) {
            $('#varyantlar.tab-pane').html(data);
            AjaxModalInit();
        }
    })
}
...
function AjaxModalInit()
{
    ...
    $('[data-stockekle]').on('click', function() {
        var section = $(this).parents('#seform');
        var product_id = $(this).data('product-id');
        var form = $('.form-control',section).serialize() +'&product_id=' + product_id;
        $.post(baseURL + 'products/addmodelstock', form , function(data) {
            if(data.status != 'success') {
                swal.fire('HATA !',data.message , 'error');
            } else rebuildvariants(product_id);
        })
    });

    $('[data-msremove]').on('click', function() {
        ...
        swal.fire({
            ...
            if(result.value) {
                $.post(baseURL + 'products/removemodelstock', {'id':id} , function(data) {
                    if(data.status != 'success') {
                        swal.fire('HATA !',data.message , 'error');
                    } else rebuildvariants(data.product_id);
                })
            }
        });
...
function openWinBuyer(id) {
    $.get(baseURL + 'reports/pro_buyer_pdf/',{'id':id}, function(data) {

        $.each(data, function(index,val) {
            var randomnumber = Math.floor((Math.random()*100)+1);
            window.open(val.url, randomnumber);
        });
    })
}

function openWinSeller(id) {
    $.get(baseURL + 'reports/pro_seller_pdf/',{'id':id}, function(data) {
